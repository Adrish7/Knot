...
- **Sidebar thread**: the one decorative element in the chrome. A 1.5px thread in the app icon's ramp (ice → sky → cyan → blue → indigo → violet, `--knot-1…6`, top to bottom) frames the sidebar on all four sides, 2.5px in from the edge so it sits inside the window rim, rounded at all four corners. It carries a soft inward glow (stronger in dark mode) and a comet of light laps it about every fourteen seconds; reduced motion hides the pulse. The thread replaces the sidebar's right hairline, and the sliver outside it is painted in `--bg` so it reads as part of the content ground. It is a preference ("Sidebar frame" in the settings popover, on by default); when off, the plain hairline returns. Nothing else in the chrome may be colored. Lives in `SidebarThread.tsx`.
- **Sidebar**: 244px, vibrancy, traffic-light space, search field on top, view rows with colored icons, list rows with a progress ring in the list color, Completed and Recently deleted after a hairline, footer with "New list" and a settings gear that opens a popover (appearance, open at login, update, shortcuts). Collapsed rail at 88px (the traffic lights sit centred between the thread on both sides) keeps icons and rings.
