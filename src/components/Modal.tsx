  ...
  return (
    <div className="modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}>
      <form className="modal-card" role="dialog" aria-modal="true" aria-labelledby="create-list-title" onSubmit={(event) => { event.preventDefault(); if (name.trim()) onCreate(name.trim(), color) }}>
        ...
        <p>You can move tasks between lists at any time.</p>
        <label className="modal-field"><span>List name</span><input ref={input} value={name} maxLength={120} onChange={(event) => setName(event.target.value)} placeholder="e.g. Side project" /></label>
