...
function sortTasks(tasks: Task[], mode: SortMode) {
  return [...tasks].sort((a, b) => {
    if (mode === 'date') return compareByDue(a, b)
...
export function Board(props: BoardProps) {
  ...
  const listNameFor = (task: Task) => (mode === 'smart' ? lists.find((list) => list.id === task.listId)?.name : undefined)
  ...
  const dropOnList = (event: React.DragEvent, listId: string) => {
    ...
    if (draggedTask) {
      const beforeTaskId = findBeforeTask(event.currentTarget as HTMLElement, event.clientY, draggedTask)
      if (listId === VIEW_DROP_KEY) props.onReorderTask?.(draggedTask, beforeTaskId)
      else props.onMoveTask(draggedTask, listId, beforeTaskId ?? undefined)
    }
    finishDrag()
  }
  ...
  const edgeFor = (listId: string, open: Task[], task: Task): DropEdge | null => {
    ...
    if (dropHint.beforeTaskId === null && open.findLast((item) => item.id !== draggedTask)?.id === task.id) return 'after'
    return null
  }
  ...
  const dropOnCanvas = (event: React.DragEvent) => {
    ...
    const before = columns.find((column) => {
      if (column.dataset.listId === draggedList) return false
  ...
  const defaultListId = lists.find((list) => list.id === activeListId)?.id ?? lists[0]?.id
